...
Your answer : """
        ...
        try:
            # Display a progress bar while running the subprocess
            for _ in tqdm(range(100), desc="Running Code"):
            ...
            full_error_message = e.stderr.decode()

            # Use regular expression to find the line number and the error message
            error_match = re.search(r'line (\d+).*?Error:(.*)', full_error_message, re.DOTALL)
            ...
            print(Fore.RED + f"Attempt {attempt + 1}: Error running generated code! Error: {error_message}" + Style.RESET_ALL)
